 ...
 * 每冷子代理折叠 ~68ms）。
 *
 * 健壮性（v0.5.0）：单任务超时（taskTimeoutMs，默认 30min，超时 interrupt +
 * 记 failed）、失败自动重试（maxRetries 默认 1，换新子代理）、worker 轮换
 * （tasksPerWorker 默认 12，防上下文累积污染）、per-run 事件监听（只登记
 * 本池在途子代理，零噪音、随 run 注销）。
 *
 * 实现要点：
...
const DEFAULTS = Object.freeze({
  ...
  taskTimeoutMs: 1_800_000, // 单任务超时（默认 30min；0 = 不限制）：防 worker 挂死
  maxRetries: 1, // 单任务失败自动重试次数（瞬态故障如配额/网络抖动）
  retryDelayMs: 5_000, // 重试前等待（换新子代理执行）
  tasksPerWorker: 12, // 每 worker 处理任务数上限：到点退役换新（防上下文累积污染）
})

function normalizeConfig(config) {
  const cfg = { ...DEFAULTS }
  if (config === void 0 || config === null) return cfg
  // 兼容 Web 设置里更直观的 maxConcurrency 字段名。
  if (config.maxConcurrency !== void 0 && config.defaultMaxConcurrency === void 0) {
    config = { ...config, defaultMaxConcurrency: config.maxConcurrency }
  }
...
/** dsh-parallel-pool 用户设置命名空间与 schema（Web 插件配置页可见）。 */
const PARALLEL_POOL_SETTINGS_NAMESPACE = 'dsh-parallel-pool'
const PARALLEL_POOL_SETTINGS_SCHEMA = z.object({
  maxConcurrency: z.number().step(1).min(1).max(DEFAULTS.maxConcurrencyCap).default(DEFAULTS.defaultMaxConcurrency),
})
...
function validateArgs(args) {
  ...
  if (args.taskTimeoutMs !== void 0) {
    const n = Number(args.taskTimeoutMs)
    if (!Number.isInteger(n) || n < 0) {
      throw new Error('parallel_pool: taskTimeoutMs must be a non-negative integer (ms; 0 = no limit)')
    }
  }
  if (args.maxRetries !== void 0) {
    const n = Number(args.maxRetries)
    if (!Number.isInteger(n) || n < 0 || n > 3) {
      throw new Error('parallel_pool: maxRetries must be an integer in 0..3')
    }
  }
}
...
/** 可自动重试的结算原因（瞬态故障）；refusal/aborted/completed 不重试。 */
const RETRIABLE_STOP_REASONS = new Set(['error', 'max-tokens', 'timeout'])
...
/**
 * 带单任务超时的结算等待：到点触发 onTimeout（interrupt 子代理）并返回
 * stopReason 'timeout' 终态。timeoutMs <= 0 时等价于 waitForChildEnd。
 */
async function waitForChildEndOrTimeout(endByRun, childId, signal, timeoutMs, onTimeout) {
  if (!(timeoutMs > 0)) return waitForChildEnd(endByRun, childId, signal)
  let timer
  const timeout = new Promise((resolve) => {
    timer = setTimeout(() => {
      try { onTimeout(childId) } catch { /* 尽力而为 */ }
      resolve({ sessionId: childId, stopReason: 'timeout', lastAssistantMessage: [] })
    }, timeoutMs)
  })
  try {
    return await Promise.race([waitForChildEnd(endByRun, childId, signal), timeout])
  } finally {
    clearTimeout(timer)
  }
}
...
function renderResultLine(entry) {
  ...
  const attempts = entry.attempts !== undefined && entry.attempts > 1 ? ` ×${entry.attempts}` : ''
  const note = entry.error !== void 0 ? ` — ${entry.error}` : ''
  return `#${entry.index + 1} ${entry.description} ${icon} ${entry.status} ${seconds}s${attempts}${note}`
}
...
/** 池引擎：工作器池滚动调度整批任务，全部结算（或中止）后返回汇总。 */
export async function runPool(ctx, cfg, engine) {
  const { tasks, maxConcurrency, provider, failFast, parent, signal, onProgress, onTaskSettled } = engine
  const results = new Array(tasks.length)
  const startedAt = Date.now()
  const activeChildren = new Set() // in-flight 子会话 id（中止时逐个 interrupt）
  const endByRun = new Map() // 本池子会话结算登记（仅登记 activeChildren，零噪音）
  let next = 0
  let peakActive = 0
  let settled = 0
  ...
  try {
    ...
    /** 构建结果条目（timeout 单列错误文案）。 */
    const buildEntry = (task, index, t0, end, attempt) => {
      ...
      if (stopReason === 'timeout') {
        const timeoutLabel = cfg.taskTimeoutMs >= 60_000
          ? `${Math.round(cfg.taskTimeoutMs / 60_000)}m`
          : `${Math.round(cfg.taskTimeoutMs / 1000)}s`
        entry.error = `task timed out after ${timeoutLabel}`
        entry.timedOut = true
      } else if (status !== 'completed') {
        entry.error = stopReasonError(stopReason)
      }
    ...
    const record = (entry) => {
      ...
      if (onTaskSettled !== undefined) onTaskSettled(entry, results, settled)
    ...
    /** 派发一个任务回合（必要时新建/复用 worker 子代理）并等待结算。 */
    const dispatchTurn = async (task, childState) => {
      if (childState.childId === undefined) {
        // 新 worker：首任务随 startContinuable 派发。
        const label = `pool worker ${childState.workerIndex + 1}`
        ...
        childState.childId = start.childId
        activeChildren.add(childState.childId)
      } else {
        // 复用 worker：清理上一回合可能残留的登记，再 followup 派发。
        endByRun.delete(childState.childId)
        await ctx.subagents.followup(parent, childState.childId, [{ type: 'text', text: task.prompt }], {
          source: { kind: 'coordinator', form: 'relay', senderSessionId: parent.id },
          signal,
        })
      }
      const end = await waitForChildEndOrTimeout(endByRun, childState.childId, signal, cfg.taskTimeoutMs, (id) => {
    ...
    /** 退役当前 worker 子代理（超时/失败/轮换后换新，避免上下文与状态残留）。 */
    const retireChild = (childState) => {
      if (childState.childId === undefined) return
      activeChildren.delete(childState.childId)
      childState.childId = undefined
      childState.tasksDone = 0
    }

    /**
     * 单个工作器：串行处理任务；失败可重试（换新子代理）、超时中断、
     * 每 tasksPerWorker 个任务轮换新子代理（防上下文累积污染）。
     */
    const runWorker = async (workerIndex) => {
      const childState = { workerIndex, childId: undefined, tasksDone: 0 }
      ...
      try {
        for (;;) {
          ...
          try {
            for (;;) {
              attempt += 1
              end = await dispatchTurn(task, childState)
              const retriable = RETRIABLE_STOP_REASONS.has(end.stopReason ?? '')
              if (retriable && attempt <= cfg.maxRetries && !signal.aborted && !stopDispatch) {
                // 换新子代理重试（被中断/失败过的 child 不复用，避免残留事件竞态）。
                retireChild(childState)
                if (cfg.retryDelayMs > 0) await sleepAbortable(cfg.retryDelayMs, signal)
                continue
              }
              break
            }
          ...
          const entry = buildEntry(task, index, t0, end, attempt)
          if (entry.status === 'failed') {
            const detail = await childFailureDetail(ctx, childState.childId)
          ...
          childState.tasksDone += 1
          if (childState.tasksDone >= cfg.tasksPerWorker) retireChild(childState)
          index = claim()
          if (index < 0) break
        }
      } finally {
...
/** 单任务完成通知：每个子任务结算后立即投递给主对话。 */
function fitTaskNotice(jobId, entry, total, cfg) {
  const head = `parallel_pool task ${entry.index + 1}/${total} ${entry.status}`
  const line = renderResultLine(entry)
  const output = entry.output !== void 0 && entry.output.length > 0 ? `\n${entry.output}` : ''
  const full = `${head} (job ${jobId}):\n${line}${output}`
  if (full.length <= cfg.noticeMaxChars) return full
  return `${full.slice(0, cfg.noticeMaxChars)}\n[notice truncated — read the full results with job_output ${jobId}]`
}
...
export function apply(ctx, config = {}) {
  ...
  // Web 插件配置：注册 settings 命名空间，用户可配置默认并发数。
  // 直接使用 ctx.settings 服务，避免新增 loader 难以解析的包依赖。
  let getCfg = () => cfg
  ctx.inject(['settings'], (sctx) => {
    const scope = sctx.settings.register(PARALLEL_POOL_SETTINGS_NAMESPACE, PARALLEL_POOL_SETTINGS_SCHEMA, { base: cfg })
    getCfg = () => normalizeConfig({ ...cfg, ...scope.get() })
    sctx.effect(() => () => { getCfg = () => cfg })
    scope.watch(() => { getCfg = () => normalizeConfig({ ...cfg, ...scope.get() }) })
  })
  ...
  ctx.effect(() => ctx.jobs.onJobDone((snapshot, _owner) => {
    ...
    const message = createUserMessage({
      content: [{ type: 'text', text: fitNotice(snapshot.id, summary, getCfg()) }],
      source: {
        kind: 'plugin',
  ...
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    text: 'For 2+ independent tasks, submit the whole batch to parallel_pool in ONE call instead of fanning out manual subagent waves. It is always non-blocking: returns a job id immediately (your turn does not block) and dispatches subagents with a rolling window — each finished task is immediately replaced by the next queued task, so slow tasks never stall the batch. Each completed task is delivered to the conversation as it settles, and a complete summary arrives when the batch finishes. Keep working on independent steps; job_output shows live per-task progress; job_kill stops the batch and partial results are still delivered. The background parameter is accepted for compatibility but the pool never blocks inline.',
  }), 'dsh-parallel-pool: prompt section')

  ctx.effect(() => ctx.tools.register(defineTool({
    name: cfg.toolName,
    description:
      'Submit a batch of independent tasks to a dynamic subagent pool. The pool dispatches up to maxConcurrency subagents with a rolling window — as soon as any task finishes, the next queued task starts immediately (no round barriers), so slow tasks never stall the batch. It is always non-blocking: returns a job id immediately and delivers each completed task as an in-session message when it settles, plus a final summary when the batch finishes. job_output shows live per-task progress; job_kill stops the batch. The background parameter is accepted for compatibility but the pool never blocks inline.',
    parameters: {
      tasks: {
        type: 'array',
        required: true,
      ...
      background: {
        type: 'boolean',
        description: `Accepted for backward compatibility. The pool is always non-blocking: it returns a job id immediately and pushes per-task results as they settle. Default ${cfg.backgroundDefault}; false no longer waits inline.`,
      },
      taskTimeoutMs: {
        type: 'integer',
        description: `Per-task timeout in ms: a task that exceeds it is interrupted, marked failed, and retried per maxRetries. Default ${cfg.taskTimeoutMs} (30 min); 0 disables.`,
      },
      maxRetries: {
        type: 'integer',
        description: `Automatic retries per failed task (transient failures) on a fresh subagent. Default ${cfg.maxRetries}; 0..3.`,
      },
    },
    output: {
      schema: {
      ...
      render: (_args, value) => [{ type: 'text', text: `parallel_pool: batch submitted as background job ${value.jobId} — each completed task will be delivered to the conversation as it settles; the final summary arrives when the batch finishes. Keep working on independent steps. (job_output shows live per-task progress; job_kill stops the batch.)` }],
    },
    isConcurrencySafe: () => true,
    presentCall: (args) => ({
      card: 'generic',
      title: `pool: ${args.tasks.length} tasks × ${args.maxConcurrency ?? cfg.defaultMaxConcurrency} (bg)`,
      kind: 'execute',
    ...
    async execute(args, exec) {
      ...
      // 每次调用读取最新配置，Web 设置里的并发数修改后下一次调用即生效。
      const cfg = getCfg()
      ...
      const taskTimeoutMs = args.taskTimeoutMs ?? cfg.taskTimeoutMs
      const maxRetries = clamp(args.maxRetries ?? cfg.maxRetries, 0, 3)
      ...
      // per-call 配置覆盖：工具入参可覆写超时/重试默认值。
      const callCfg = { ...cfg, taskTimeoutMs, maxRetries }

      // 无论 background 参数为何值，都使用非阻塞后台任务：
      // 提交即返回 jobId；每个子任务结算时向主对话增量推送，整批结束后再推送汇总。
      const state = {
        summary: undefined,
      ...
      jobId = ctx.jobs.start({
        ...
        run: () => {
          ...
          const deliverTask = (entry) => {
            const message = createUserMessage({
              content: [{ type: 'text', text: fitTaskNotice(jobId, entry, tasks.length, getCfg()) }],
              source: {
                kind: 'plugin',
                plugin: 'dsh-parallel-pool',
                form: 'task-progress',
                summary: `task ${entry.index + 1}/${tasks.length} ${entry.status}`.slice(0, 40),
              },
            })
            try {
              ...
              ctx.logger?.warn?.(`dsh-parallel-pool: per-task delivery failed for job ${jobId}: ${String(error)}`)
            }
          }
          return {
            cancel: (reason) => {
