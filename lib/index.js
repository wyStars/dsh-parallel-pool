 ...
 *   continuable 子代理（对齐默认 subagent 工具：子代理面板实时可见、可续跑、
 *   平台原生完成通知）；subagent/end 事件结算 {stopReason, lastAssistantMessage}。
 * - 结算回调触发 refill()：active < maxConcurrency 且队列非空即补位。
 ...
 * - 中止时 stopRefill + 逐个 interrupt in-flight 子代理。
 */
...
async function runPool(ctx, cfg, engine) {
  const { tasks, maxConcurrency, provider, failFast, parent, signal, endByRun } = engine
  ...
  const launch = (index) => {
    ...
    return (async () => {
      ...
      let childId
      try {
        // continuable 子代理（对齐默认 subagent 工具）：面板可见、可续跑、原生通知。
  ...
  if (callAborted) {
    stopRefill = true
    // 中断 in-flight 子代理（continuable 支持 interrupt；父代理即权威凭据）。
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    text: 'For 2+ independent tasks, submit the whole batch to parallel_pool in ONE call instead of fanning out manual subagent waves: it runs in the background by default (returns a job id immediately, so your turn does not block) and dispatches subagents with a rolling window — each finished task is immediately replaced by the next queued task, so slow tasks never stall the batch. The complete per-task results and timing arrive as an in-session message when the batch settles; keep working on independent steps meanwhile and do not poll. job_output and job_kill work on the returned job id. Set background: false only when your next action depends on the results.',
  }), 'dsh-parallel-pool: prompt section')

  ctx.effect(() => ctx.tools.register(defineTool({
    name: cfg.toolName,
    description:
      'Submit a batch of independent tasks to a dynamic subagent pool. The pool dispatches up to maxConcurrency subagents with a rolling window — as soon as any task finishes, the next queued task starts immediately (no round barriers), so slow tasks never stall the batch. Runs in the background by default: returns a job id at once and delivers the complete per-task results and timing as an in-session message when the batch settles; the turn does not block. Use it instead of manual subagent waves when you have 2+ independent tasks. job_output and job_kill work on the job id.',
    parameters: {
      tasks: {
        type: 'array',
        required: true,
    ...
    output: {
      ...
      render: (_args, value) => value.kind === 'background'
        ? [{ type: 'text', text: `parallel_pool: batch submitted as background job ${value.jobId} — the dynamic pool is running with rolling refill; the complete results will arrive as an in-session message when the batch settles. Keep working on independent steps, do not poll. (job_output and job_kill work on this id.)` }]
